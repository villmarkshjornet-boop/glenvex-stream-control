  ...
  return (
    <aside className="w-52 min-h-screen bg-g-sidebar border-r border-g-border flex flex-col flex-shrink-0">
      <div className="px-5 py-4 border-b border-g-border">
        ...
        <div className="text-[9px] text-g-muted tracking-[0.4em] uppercase mt-0.5">Creator OS</div>
      </div>

      <nav className="flex-1 overflow-y-auto py-2 px-2 space-y-3">
        {NAV_GRUPPER.map(gruppe => (
          <div key={gruppe.gruppe}>
            <p className="text-[8px] text-g-muted/40 font-bold tracking-[0.3em] uppercase px-2 py-1">
              {gruppe.gruppe}
            </p>
              ...
              {gruppe.items.map(item => {
                ...
                return (
                  <Link key={item.href} href={item.href}
                    className={`flex items-center gap-2 px-2.5 py-1.5 rounded text-xs transition-all duration-150 group ${
                      active
                  ...
                  >
                    <span className={`text-[12px] w-4 text-center flex-shrink-0 ${active ? 'text-g-green' : 'text-g-muted group-hover:text-g-text'}`}>
                      {item.icon}
                    </span>
      ...
      <div className="px-4 py-3 border-t border-g-border">
        <p className="text-[8px] text-g-muted/40 text-center tracking-widest uppercase">v3.0</p>
      </div>
    </aside>
  );
}
