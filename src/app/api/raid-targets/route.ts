...
export async function GET() {
  ...
  if (!twitchLogin) {
    void db?.from('system_events').insert({
      ...
      title:        'Raid-manager: workspace mangler twitch_login — kjør Repair i admin',
      severity:     'warning',
      metadata:     { wsId, field: 'twitch_login' },
    });
    return NextResponse.json({ targets: [], reason: 'Twitch ikke koblet — kjør Repair i admin-panelet' });
  }
