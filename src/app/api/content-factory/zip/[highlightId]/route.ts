...
export async function GET(
  ...
  // ─── Klipp-kvalitetsdata (V2) ────────────────────────────────────────────────

  const qualityMetadata = {
    highlight_id:               highlightId,
    vod_id:                     h.vod_id,
    category:                   h.category ?? null,
    clip_quality_score:         h.clip_quality_score ?? null,
    clip_quality_entertainment: h.clip_quality_entertainment ?? null,
    clip_quality_emotion:       h.clip_quality_emotion ?? null,
    clip_quality_surprise:      h.clip_quality_surprise ?? null,
    clip_quality_viral:         h.clip_quality_viral ?? null,
    clip_quality_story_arc:     h.clip_quality_story_arc ?? null,
  };
  zip.file('quality_metadata.json', JSON.stringify(qualityMetadata, null, 2), { compression: 'DEFLATE' });
