      ...
      {loading ? (
        ...
        <>
          ...
          <div className="bg-g-card border border-g-border rounded-2xl p-6 space-y-4">
            <p className="text-xs font-semibold tracking-widest uppercase text-g-muted">Community Health</p>
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-g-muted mt-1.5">
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                <div key={l as string} className="text-center p-3 bg-g-bg border border-g-border rounded-xl">
                  <p className="text-[11px] text-g-muted uppercase tracking-widest font-bold">{l}</p>
          ...
          {status.varsler.length > 0 && (
            <div className="bg-g-card border border-yellow-400/20 rounded-2xl p-6">
              <p className="text-xs font-semibold tracking-widest uppercase text-yellow-400 mb-3">Varsler</p>
              {status.varsler.map((v, i) => (
                <p key={i} className="text-sm text-g-text mb-1 flex gap-2">
          ...
          <div className="bg-g-card border border-g-border rounded-2xl p-6">
            <p className="text-xs font-semibold tracking-widest uppercase text-g-muted mb-3">Siste moderasjonslogg</p>
            {status.siste.length === 0 ? (
              <p className="text-sm text-g-muted">Ingen hendelser registrert.</p>
            ) : status.siste.map((s, i) => (
              <div key={i} className="flex items-center gap-2 py-1.5 border-b border-g-border/30 last:border-0">
                <span className="text-[11px] text-g-muted font-mono">{s.tid}</span>
                <span className="text-[11px] text-g-muted">{s.type}</span>
                <span className="text-sm text-g-text">{s.melding}</span>
              </div>
            ))}
          </div>
        </>
