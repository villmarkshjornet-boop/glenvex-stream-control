    ...
    <div className="max-w-3xl mx-auto space-y-5">
      <PageHeader title="Raid Manager" subtitle="AI-anbefalte raid-mål basert på kategori, størrelse og match-score">
        <div className="flex items-center gap-2">
          {lastUpdated && (
            <span className="text-xs text-g-muted/50">Oppdatert {tidSiden(lastUpdated.toISOString())}</span>
          )}
          <button
            onClick={() => fetchTargets(true)}
            disabled={refreshing || loading}
            className={`px-2.5 py-1.5 border rounded-lg text-[11px] transition-all ${
      ...
      {!isLive && !loading && (
        <div className="bg-g-card border border-g-border rounded-2xl p-6 text-center">
          ...
          <p className="text-[11px] text-g-muted/60 mt-1">Oppdaterer automatisk hvert 5. minutt.</p>
        </div>
      )}
      ...
      {targets.length > 0 && (
        <>
          <div className="flex items-center gap-2">
            ...
            <span className="text-[11px] text-g-muted/40">· Oppdateres hvert 60s</span>
          </div>
                ...
                <div className="flex items-start justify-between">
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      {i === 0 && <span className="text-[11px] px-1.5 py-0.5 bg-g-green/10 border border-g-green/30 rounded text-g-green font-bold">TOPP ANBEFALING</span>}
                      <p className="text-sm font-bold text-g-text">{t.username}</p>
                      <span className={`text-xs px-2 py-0.5 rounded-full border font-bold ${t.score >= 80 ? 'text-g-green border-g-green/30 bg-g-green/10' : t.score >= 60 ? 'text-yellow-400 border-yellow-400/30 bg-yellow-400/10' : 'text-g-muted border-g-border bg-g-bg'}`}>
                        {t.score}% match
                      </span>
                    </div>
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-xs text-g-muted">seere</p>
                  </div>
                </div>
