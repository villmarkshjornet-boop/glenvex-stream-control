...
import type { CommunityMember } from '@/app/api/admin/economy/users/route';
...
type AdminTab = 'workspaces' | 'creator';
type EconomyFilter = 'all' | 'errors' | 'economy' | 'bot';
...
function sevColor(sev?: string): string {
  ...
  if (sev === 'info')    return 'text-g-green';
  return 'text-g-muted';
}
...
function DetailSidebar({
  ...
  const handleRepair = async (forceAlpha = false) => {
    ...
    try {
      ...
      setRepairResult({ ok: false, error: err instanceof Error ? err.message : 'Ukjent feil' });
    } finally {
      setRepairing(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex" onClick={onClose}>
      ...
      >
          ...
          <div className={`mx-4 mt-2 mb-1 p-2 rounded text-[11px] border ${liveCheckResult.isLive ? 'border-red-500/30 bg-red-500/5 text-red-400' : liveCheckResult.error ? 'border-red-500/30 bg-red-500/5 text-red-400' : 'border-g-border bg-g-bg/50 text-g-muted'}`}>
            {liveCheckResult.error
              ...
              : liveCheckResult.isLive
                ? `Live — ${liveCheckResult.stream?.title?.slice(0, 50) ?? ''} · ${liveCheckResult.stream?.viewerCount ?? 0} seere`
                : `Offline — sjekket ${new Date(liveCheckResult.checkedAt).toLocaleTimeString('no-NO')}`
            }
          </div>
        )}
        ...
        {ws.lastStream?.metadata?.game && (
          <Row label="Spill" value={String(ws.lastStream.metadata.game)} />
        )}
        {ws.lastStream?.metadata?.viewerCount !== undefined && (
          <Row label="Peak viewers" value={String(ws.lastStream.metadata.viewerCount)} />
        )}
        ...
        {ws.audienceHb?.metadata && (
          <>
            <Row label="Observerte brukere" value={String(ws.audienceHb.metadata.totalObserved ?? '–')} />
            <Row label="Abonnenter" value={String(ws.audienceHb.metadata.subscribers ?? '–')} />
            <Row label="Aktive chattere" value={String(ws.audienceHb.metadata.activeChattters ?? '–')} />
            <Row label="Viewers sist sett" value={String(ws.audienceHb.metadata.lastViewerCount ?? '–')} />
          </>
        )}
        {!ws.audienceHb && ws.audienceSnapshot && (
          <Row label="Siste snapshot" value={`${String(ws.audienceSnapshot.metadata?.total ?? 0)} brukere (${timeAgo(ws.audienceSnapshot.created_at)})`} />
        )}
        ...
        {ws.coachReport ? (
          <>
            ...
            <Row label="Score" value={ws.coachReport.metadata?.score !== undefined ? `${ws.coachReport.metadata.score} / 100` : '–'}
              color={(ws.coachReport.metadata?.score as number) >= 70 ? 'text-g-green' : (ws.coachReport.metadata?.score as number) >= 40 ? 'text-yellow-400' : 'text-red-400'} />
            {ws.coachReport.metadata?.grade && <Row label="Grade" value={String(ws.coachReport.metadata.grade)} />}
          </>
        ) : (
...
interface CoinAdjustState {
  ...
  reason: string;
  loading: boolean;
  result: { ok: boolean; new_balance?: number; dm_queued?: boolean; error?: string } | null;
}

function UserEconomyRow({
  member,
  onBalanceUpdate,
}: {
  member: CommunityMember;
  onBalanceUpdate: (discordId: string, newBalance: number) => void;
}) {
  const [adj, setAdj] = useState<CoinAdjustState>({
    open: false, amount: '', reason: '', loading: false, result: null,
  });

  const handleSubmit = async () => {
    const parsedAmount = parseInt(adj.amount, 10);
    if (!Number.isInteger(parsedAmount) || parsedAmount === 0) return;
    if (!adj.reason.trim()) return;

    setAdj(prev => ({ ...prev, loading: true, result: null }));
    try {
      const res = await fetch('/api/admin/economy/adjust', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ discord_id: member.discord_id, amount: parsedAmount, reason: adj.reason.trim() }),
      });
      const data = await res.json();
      if (data.ok) {
        onBalanceUpdate(member.discord_id, data.new_balance);
        setAdj(prev => ({ ...prev, loading: false, result: data, amount: '', reason: '' }));
      } else {
        setAdj(prev => ({ ...prev, loading: false, result: { ok: false, error: data.error ?? 'Feil' } }));
      }
    } catch (err: unknown) {
      setAdj(prev => ({
        ...prev,
  ...
  const displayName = member.display_name ?? member.username ?? member.discord_id;

  return (
    <>
      <tr className="border-b border-g-border/20 last:border-0 hover:bg-white/[0.015] transition-colors">
        <td className="py-3 pr-3">
          <div className="flex items-center gap-2">
            {member.discord_avatar_url && (
              <img src={member.discord_avatar_url} alt="" className="w-6 h-6 rounded-full flex-shrink-0" />
            )}
            <div>
              <p className="text-xs font-semibold text-g-text leading-tight">{displayName}</p>
              {member.username && member.display_name && member.username !== member.display_name && (
                <p className="text-[11px] text-g-muted">{member.username}</p>
              )}
            </div>
          </div>
        </td>
        <td className="py-3 pr-3">
          <span className="inline-flex items-center gap-1 px-1.5 py-0.5 text-[11px] font-bold rounded bg-g-green/10 text-g-green border border-g-green/20">
            Lv {member.level}
          </span>
        </td>
        <td className="py-3 pr-3 font-mono text-xs text-g-text">{(member.xp ?? 0).toLocaleString('no-NO')}</td>
        <td className="py-3 pr-3">
          <span className="font-mono text-xs text-g-green font-bold">{(member.coins_balance ?? 0).toLocaleString('no-NO')} 🪙</span>
        </td>
        <td className="py-3 pr-3 font-mono text-xs text-g-muted">{member.streak_days ?? 0}d</td>
        <td className="py-3 pr-3 text-xs text-g-muted">{timeAgo(member.last_seen)}</td>
        <td className="py-3">
          <button
            onClick={() => setAdj(prev => ({ ...prev, open: !prev.open, result: null }))}
            className="px-2 py-1 text-[11px] font-bold border border-g-border rounded text-g-muted hover:text-g-green hover:border-g-green/30 transition-colors"
          >
            Gi coins
          </button>
        </td>
      ...
      {adj.open && (
        <tr className="border-b border-g-border/20">
          <td colSpan={7} className="pb-3 pt-1 pr-3">
            <div className="bg-g-bg border border-g-border/50 rounded-xl p-3 space-y-2">
              <p className="text-[11px] text-g-muted font-semibold uppercase tracking-widest">Coins-justering for {displayName}</p>
              <div className="flex gap-2">
                <input
                  type="number"
                  value={adj.amount}
                  onChange={e => setAdj(prev => ({ ...prev, amount: e.target.value }))}
                  placeholder="Beløp (neg. for trekk)"
                  className="w-36 bg-g-bg border border-g-border rounded-lg px-3 py-2 text-sm text-g-text focus:outline-none focus:border-g-green/40 font-mono"
                />
                <input
                  type="text"
                  value={adj.reason}
                  onChange={e => setAdj(prev => ({ ...prev, reason: e.target.value }))}
                  placeholder="Grunn (vises i DM)"
                  className="flex-1 bg-g-bg border border-g-border rounded-lg px-3 py-2 text-sm text-g-text focus:outline-none focus:border-g-green/40"
                />
                <button
                  onClick={handleSubmit}
                  disabled={adj.loading || !adj.amount || !adj.reason.trim()}
                  className="px-3 py-2 bg-g-green/10 border border-g-green/30 rounded-lg text-xs font-bold text-g-green hover:bg-g-green/20 transition-colors disabled:opacity-40"
                >
                  {adj.loading ? 'Sender…' : 'Bekreft'}
                </button>
              </div>
              {adj.result && (
                <p className={`text-[11px] font-semibold ${adj.result.ok ? 'text-g-green' : 'text-red-400'}`}>
                  {adj.result.ok
                    ? `✓ Oppdatert — ny saldo ${(adj.result.new_balance ?? 0).toLocaleString('no-NO')} 🪙 · ${adj.result.dm_queued ? '✓ DM i kø' : '⚠ DM feilet, coins oppdatert'}`
                    : `✗ ${adj.result.error}`}
                </p>
              )}
            </div>
...
function CreatorControlRoom() {
  ...
  const [evFilter, setEvFilter] = useState<EconomyFilter>('all');

  // Spam/compliance state
  const [blocked, setBlocked] = useState<EventRow[]>([]);
  const [blockedLoading, setBlockedLoading] = useState(true);

  // Bot control state
  const [botResults, setBotResults] = useState<Record<string, { ok: boolean; message?: string; error?: string } | null>>({});
  const [botLoading, setBotLoading] = useState<Record<string, boolean>>({});

  const loadMembers = useCallback(async () => {
    setMembersLoading(true);
    try {
      const res = await fetch('/api/admin/economy/users');
      if (res.ok) {
  ...
  const loadEvents = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/system-events?minutesBack=1440&limit=50');
      if (res.ok) {
        const d = await res.json();
        setEvents(d.events ?? []);
      }
  ...
  const loadBlocked = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/system-events?minutesBack=1440&limit=20&source=compliance');
      if (res.ok) {
        const d = await res.json();
        setBlocked((d.events ?? []).filter((e: EventRow) => e.event_type === 'COMPLIANCE_BLOCKED'));
      }
    } catch { /* ignore */ } finally {
  ...
  useEffect(() => {
    ...
    loadEvents();
    loadBlocked();
  }, [loadMembers, loadEvents, loadBlocked]);

  const handleBalanceUpdate = useCallback((discordId: string, newBalance: number) => {
    setMembers(prev => prev.map(m => m.discord_id === discordId ? { ...m, coins_balance: newBalance } : m));
  }, []);

  const filteredEvents = useMemo(() => {
    if (evFilter === 'all') return events;
    if (evFilter === 'errors') return events.filter(e => e.severity === 'error' || e.severity === 'warning');
    if (evFilter === 'economy') return events.filter(e => e.event_type.toLowerCase().includes('coin') || e.event_type.toLowerCase().includes('economy') || e.source === 'economy');
    if (evFilter === 'bot') return events.filter(e => e.source === 'discord_bot' || e.source === 'twitch_bot' || e.source === 'bot');
    return events;
  }, [events, evFilter]);

  const SectionHeader = ({ title }: { title: string }) => (
    <h2 className="text-xs font-semibold tracking-widest uppercase text-g-muted mb-4 pb-3 border-b border-g-border/40">{title}</h2>
  );

  const callBotEndpoint = async (key: string, path: string) => {
    setBotLoading(prev => ({ ...prev, [key]: true }));
    setBotResults(prev => ({ ...prev, [key]: null }));
    try {
      const res = await fetch(path, { method: 'POST' });
      const data = await res.json().catch(() => ({ ok: res.ok }));
      setBotResults(prev => ({ ...prev, [key]: data }));
    } catch (err: unknown) {
      setBotResults(prev => ({
        ...prev,
        [key]: { ok: false, error: err instanceof Error ? err.message : 'Nettverksfeil' },
      }));
    } finally {
      setBotLoading(prev => ({ ...prev, [key]: false }));
    }
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="glass-card rounded-2xl p-6 mb-4">
        <SectionHeader title="Bruker-økonomi" />
        {membersLoading && (
          <div className="space-y-3">
            {[0,1,2].map(i => (
              <div key={i} className="h-9 bg-g-bg/60 rounded animate-pulse" />
            ))}
          </div>
        )}
        {!membersLoading && members.length === 0 && (
          <p className="text-xs text-g-muted py-4 text-center">Ingen community-medlemmer ennå</p>
        )}
        {!membersLoading && members.length > 0 && (
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-g-border/40">
                  {['Display Name', 'Level', 'XP', 'Coins', 'Streak', 'Siste sett', ''].map(h => (
                    <th key={h} className="pb-2 pr-3 text-left text-g-muted/70 uppercase tracking-wider text-[11px] font-semibold whitespace-nowrap">
                      {h}
                    </th>
                  ))}
                </tr>
      ...
      {/* ── Bot-kontroll ────────────────────────────────────────────────────── */}
      <div className="glass-card rounded-2xl p-6 mb-4">
        <SectionHeader title="Bot-kontroll" />
        <div className="flex flex-wrap gap-3">
          {([
            { key: 'discord', label: 'Test Discord-melding', path: '/api/bot/test-discord' },
            { key: 'twitch',  label: 'Test Twitch-melding',  path: '/api/bot/test-twitch' },
            { key: 'sync',    label: 'Force sync',            path: '/api/bot/sync' },
          ] as const).map(({ key, label, path }) => (
            <div key={key} className="space-y-1">
              <button
                onClick={() => callBotEndpoint(key, path)}
                disabled={botLoading[key]}
                className="px-3 py-2 bg-g-bg border border-g-border rounded-lg text-xs font-bold text-g-muted hover:text-g-green hover:border-g-green/30 transition-colors disabled:opacity-40"
              >
                {botLoading[key] ? 'Kjører…' : label}
              </button>
              {botResults[key] && (
                <p className={`text-[11px] ${botResults[key]?.ok ? 'text-g-green' : 'text-red-400'}`}>
                  {botResults[key]?.ok
                    ? `✓ ${botResults[key]?.message ?? 'OK'}`
                    : `✗ ${botResults[key]?.error ?? 'Endepunkt ikke funnet — implementer /api/bot/...'}`}
                </p>
              )}
            </div>
          ))}
      ...
      {/* ── System-logger ───────────────────────────────────────────────────── */}
      <div className="glass-card rounded-2xl p-6 mb-4">
        <div className="flex items-center justify-between mb-4 pb-3 border-b border-g-border/40">
          <h2 className="text-xs font-semibold tracking-widest uppercase text-g-muted">System-logger (siste 24t)</h2>
          <div className="flex gap-1">
            {(['all', 'errors', 'economy', 'bot'] as EconomyFilter[]).map(f => (
              <button
                key={f}
                ...
                className={`px-2 py-1 text-[11px] rounded border transition-colors ${
                  evFilter === f
                    ...
                    : 'border-g-border text-g-muted hover:border-g-green/20'
                }`}
              >
                {f === 'all' ? 'Alle' : f === 'errors' ? 'Feil' : f === 'economy' ? 'Økonomi' : 'Bot'}
              </button>
            ))}
          </div>
        </div>
        {eventsLoading && <p className="text-xs text-g-muted animate-pulse py-2">Laster hendelser…</p>}
        {!eventsLoading && filteredEvents.length === 0 && (
          <p className="text-xs text-g-muted py-2">Ingen hendelser</p>
        )}
        {!eventsLoading && filteredEvents.length > 0 && (
          <div className="space-y-0">
            {filteredEvents.map((ev, i) => (
              <div key={i} className="py-2 border-b border-g-border/20 last:border-0 flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className={`text-[11px] font-mono font-bold flex-shrink-0 ${sevColor(ev.severity)}`}>
                      {ev.event_type}
                    </span>
                    <span className={`text-[11px] px-1 rounded ${
                      ev.severity === 'error'   ? 'bg-red-500/10 text-red-400' :
                  ...
                  {ev.title && (
                    <p className="text-[11px] text-g-muted/80 mt-0.5 leading-tight truncate">{ev.title.slice(0, 100)}</p>
                  )}
                </div>
                <span className="text-[11px] text-g-muted flex-shrink-0">{timeAgo(ev.created_at)}</span>
              </div>
      ...
      {/* ── Anti-spam status ────────────────────────────────────────────────── */}
      <div className="glass-card rounded-2xl p-6 mb-4">
        <SectionHeader title="Blokkerte meldinger (siste 24t)" />
        {blockedLoading && <p className="text-xs text-g-muted animate-pulse py-2">Laster…</p>}
        {!blockedLoading && blocked.length === 0 && (
          <p className="text-xs text-g-green py-2">Ingen blokkerte meldinger</p>
        )}
        {!blockedLoading && blocked.length > 0 && (
          <div className="space-y-0">
            {blocked.map((ev, i) => {
              ...
              return (
                <div key={i} className="py-2 border-b border-g-border/20 last:border-0">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0 space-y-0.5">
                      <div className="flex items-center gap-2 text-[11px]">
                        <span className="text-red-400 font-mono font-bold">{String(meta.ruleId ?? '–')}</span>
                        <span className="text-g-muted">{String(meta.channel ?? '–')}</span>
                        {!!meta.category && <span className="text-g-muted/60">· {String(meta.category)}</span>}
                      </div>
                      {!!meta.contentPreview && (
                        <p className="text-[11px] text-g-muted/70 font-mono truncate">
                          {String(meta.contentPreview).slice(0, 80)}
                        </p>
                      )}
                    </div>
                    <span className="text-[11px] text-g-muted flex-shrink-0">{timeAgo(ev.created_at)}</span>
...
export default function AdminPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (filters.errorsOnly)           ws = ws.filter(w => w.lastError !== null && ageMs(w.lastError.created_at) < 24 * 3_600_000);
  ...
  async function diagnoseUser() {
    ...
    try {
      ...
      setDiagnoseResult({ error: err instanceof Error ? err.message : 'Ukjent feil' });
    } finally {
      setDiagnosing(false);
    }
  }
  ...
  return (
    <div className="min-h-screen bg-g-bg text-g-text">
      ...
      <div className="sticky top-0 z-30 bg-g-card/90 backdrop-blur-sm border-b border-g-border px-6 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex gap-1 mr-3">
            {([['workspaces', 'Workspaces'], ['creator', 'Creator Control Room']] as [AdminTab, string][]).map(([tab, label]) => (
              <button
                key={tab}
      ...
      <div className="p-6 space-y-5 max-w-[1800px] mx-auto">
        ...
        {/* ── Creator Control Room tab ──────────────────────────────────────── */}
        {activeTab === 'creator' && <CreatorControlRoom />}
                ...
                <div className="space-y-1.5">
                  ...
                  {diagnoseResult.recentEvents?.length > 0 && (
                    <div className="bg-g-bg border border-g-border/50 rounded p-2 space-y-0.5">
                      ...
                      {diagnoseResult.recentEvents.slice(0, 5).map((e: any, i: number) => (
                        ...
                        return (
                          ...
                          >
                              ...
                              {ws.coachReport ? (
                                <div>
                                  {ws.coachReport.metadata?.score !== undefined && (
                                    <p className={`text-[11px] font-bold ${(ws.coachReport.metadata.score as number) >= 70 ? 'text-g-green' : (ws.coachReport.metadata.score as number) >= 40 ? 'text-yellow-400' : 'text-red-400'}`}>
                                      {String(ws.coachReport.metadata.score)}
                            ...
                            <td className="px-3 py-2.5 max-w-[160px]">
                              {ws.lastStream ? (
                                <div>
                                  <p className="text-[11px] text-g-text truncate">{String(ws.lastStream.metadata?.title ?? '').slice(0, 35) || '–'}</p>
                                  <p className="text-[11px] text-g-muted">{String(ws.lastStream.metadata?.game ?? '–')}</p>
