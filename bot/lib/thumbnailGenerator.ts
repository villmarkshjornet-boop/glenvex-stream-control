...
async function kjørThumbnailSyklus(): Promise<void> {
  ...
  for (const h of highlights) {
    ...
    const { buildThumbnailV8 } = require('./thumbnailBuilderV8');
    buildThumbnailV8(h.id, 'auto_after_clip')
      .catch((err: any) => {
...
export async function startThumbnailWorker(): Promise<void> {
  ...
  wLog('INFO', 'THUMBNAIL_POLL_STARTED', {
    intervallMs: POLL_MS,
    versjon: 'V8 (story-first · 200 frames · self-review · composite)',
    filter: 'clip_status=CLIPPED AND thumbnail_status=PENDING AND clip_url IS NOT NULL',
  });
...
export async function forceThumbnail(highlightId: string, source?: string): Promise<{ ok: boolean; melding: string }> {
  ...
  const { buildThumbnailV8 } = require('./thumbnailBuilderV8');
  buildThumbnailV8(highlightId, source)
    .catch(() => {})
    .finally(() => { generererNå.delete(highlightId); });

  return { ok: true, melding: `Thumbnail V8 startet for ${highlightId}` };
}
